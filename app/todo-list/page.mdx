...
## Super high priority 
- [ ] Create two pages, one for the user sign-up and another for the instructor sign-up. 
  - [ ] Both pages should have the sign up form from clerk, and should create a new user 
  in the database with the role of user or instructor, respectively, or add the new role 
  if the user already exists. 
  - [ ] The sign-up pages should redirect to the respective dashboard page after the user
  signs up, or if the user is already signed in. 
...
- [ ] Add a page to create or edit the user profile. 
- [ ] Add a model to store the user profile. 
- [ ] Create an endpoint to manage the user profile. 
- [x] Create a thumbnail field in the course model, with w-240 w-135 

- [ ] Move the delete button of the course edition page to the course list page,
      next to the edit button.
- [x] Check all of the endpoints and urls, so they are consistent and
      predictable.
...
- Add a login button in navbar
- refactor the course list page to show the courses using the fetch API and make smaller components.
